  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {MAIN_MODULES.map((module) => {
            ...
            return (
              <div 
                key={module.id} 
                className={`bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 overflow-hidden transition-all duration-300 shadow-sm flex flex-col group ${
              ...
              >
                ...
                <div className="p-6 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-lg font-black text-slate-800 dark:text-slate-100 tracking-tight uppercase leading-none">{module.name}</h2>
                      <p className="text-[9px] font-black text-indigo-500 uppercase tracking-widest mt-1">Module {module.id}</p>
                    </div>
                  </div>
                  {moduleRestricted ? (
                    <span className="text-[9px] font-black text-rose-500 uppercase tracking-wider bg-rose-50 dark:bg-rose-950/30 border border-rose-100 dark:border-rose-900/40 px-2 rounded-full flex items-center gap-1">
                      <Icon name="lock" size={10} /> Locked
                    </span>
                  ) : (
                    <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 px-2.5 py-1 rounded-full shadow-sm">
                ...
                <div className="p-6 space-y-2.5 flex-1 bg-white dark:bg-slate-900">
                  {submodules.map(sub => {
                    ...
                    return (
                      <button 
                        ...
                        className={`w-full text-left p-3.5 rounded-2xl font-bold text-xs flex items-center justify-between transition-all border outline-none ${
                          subRestricted 
                            ? 'bg-slate-50 dark:bg-slate-950/40 text-slate-300 dark:text-slate-600 border-slate-100 dark:border-slate-850 cursor-not-allowed opacity-60' 
                            : 'bg-slate-50 dark:bg-slate-950/20 border-slate-100 dark:border-slate-800/80 text-slate-600 dark:text-slate-300 hover:bg-slate-900 dark:hover:bg-white hover:text-white dark:hover:text-slate-900 hover:border-slate-900 dark:hover:border-white group-hover:border-slate-200 dark:group-hover:border-slate-700 shadow-sm hover:shadow-md hover:translate-x-1.5'
                        }`}
                      >
                        <span className="flex items-center gap-2.5 min-w-0">
                          <span className={`px-2 py-1 rounded-lg text-[9px] font-black transition-colors ${
                            subRestricted 
                              ...
                              : 'bg-white dark:bg-slate-900 text-indigo-600 group-hover:bg-slate-800 dark:group-hover:bg-slate-100 border border-slate-200 dark:border-slate-800'
                          }`}>
                            {sub.id}
                          </span>
                        ...
                        {subRestricted ? (
                          <Icon name="lock" size={12} className="text-slate-300 dark:text-slate-600 flex-shrink-0" />
                        ) : (
                          <div className="w-5 h-5 rounded-full hover:bg-white/20 flex items-center justify-center text-slate-400 dark:text-slate-500 group-hover:text-white transition-colors">
      ...
      <div className="text-center pt-4">
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-100/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-full border border-slate-200/50 dark:border-slate-800/80 text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-widest shadow-sm">
